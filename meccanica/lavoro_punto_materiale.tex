\chapter{Lavoro ed energia per il punto materiale}
...
\subsection{Campo di forze gravitazionali}
...
\subsection{Campo di forze elastiche}

In questo caso $\vect{F} (x) = - k x \vect{i}$ e l'energia potenziale elastica sarà pari a

\begin{equation}
    \highlight{U(x)} = - \int_{x_0}^x (- k x) dx + U(x_0) = \highlight{\frac{1}{2} k x^2} + \mathrm{const}
\end{equation}
...
\section{Conservazione dell'energia meccanica con forze conservative}
...
\section{Variazione dell'energia meccanica in presenza di forze non conservative}
